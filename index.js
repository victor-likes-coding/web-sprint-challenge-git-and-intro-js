...
function addArtist(arr, ...rest) {
  const [name, years, genre, nationality, bio] = rest;
  const artist = {
    name,
    years,
    genre,
...
function lotsOfArt(arr) {
  /*Your Code Here*/
  return arr.filter((artist) => artist.paintings > 100).map((artist) => artist.name);
}
...
function artistInfo(arr, name) {
  /*Your Code Here*/
  return arr.filter((artist) => artist.name === name)[0].bio;
}
...
function artistByCountry(arr, nationality) {
  /*Your Code Here*/
  return arr.filter((artist) => nationality === artist.nationality).map((artist) => artist.name);
}
